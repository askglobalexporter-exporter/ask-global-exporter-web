...
export function AdminLivePreview({ kind, label = "Preview langsung" }: { kind: PreviewKind; label?: string }) {
  ...
  return <div className={`admin-live-preview admin-live-preview-${kind}`} ref={root}>
    ...
    </div> : kind === "homepage" ? <div className={`admin-public-section-preview ${data.section_key === "hero" ? "is-hero" : ""}`} style={data.section_key === "hero" && validImage(image) ? {backgroundImage:`linear-gradient(90deg,rgba(4,24,16,.94),rgba(4,24,16,.2)),url("${image}")`} : undefined}>
