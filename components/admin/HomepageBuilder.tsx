...
export function HomepageBuilder({ initialSections }: { initialSections: Section[] }) {
  ...
  return <>
    <div className="admin-builder-list">{sections.map((section, index) => <div key={section.id} className="admin-builder-section">
      ...
      <details className="admin-builder-copy"><summary>Edit section copy</summary><form action={saveHomepageSectionContentAction} className="admin-form"><input type="hidden" name="id" value={section.id}/><input type="hidden" name="section_key" value={section.section_key}/><div className="admin-form-grid"><label><span>Eyebrow</span><input name="eyebrow" defaultValue={section.content?.eyebrow ?? ""}/></label><label><span>Heading</span><input name="title" defaultValue={section.content?.title ?? ""}/></label><label className="admin-field-full"><span>Summary</span><textarea name="summary" rows={2} defaultValue={section.content?.summary ?? ""}/></label><label className="admin-field-full"><span>Body</span><textarea name="body" rows={4} defaultValue={section.content?.body ?? ""}/></label><label className="admin-field-full"><span>Section image URL</span><input name="image_url" type="url" defaultValue={section.content?.image_url ?? ""} placeholder="https://ik.imagekit.io/…"/></label><label><span>CTA label</span><input name="cta_label" defaultValue={section.content?.cta_label ?? ""}/></label><label><span>CTA URL</span><input name="cta_url" defaultValue={section.content?.cta_url ?? ""}/></label></div><div className="admin-form-actions"><SubmitButton pendingLabel="Publishing…">Publish section copy</SubmitButton></div></form></details>
    </div>)}</div>
    <div className="admin-form-actions" style={{marginTop:18}}>{saved && <span className="admin-toast">Homepage layout saved</span>}<button type="button" className="admin-primary-button" onClick={save} disabled={pending}>{pending ? <LoaderCircle className="admin-spin" size={15} /> : <Save size={15} />}{pending ? "Saving…" : "Save layout"}</button></div>
  </>;
}
