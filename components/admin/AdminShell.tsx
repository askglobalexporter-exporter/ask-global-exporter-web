...
const navigation = [
  ...
  { href: "/admin/inquiries", label: "Inquiry CRM", icon: MessageSquareText, permission: "inquiries.read" },
  { href: "/admin/products", label: "Products", icon: Boxes, permission: "products.read" },
  { href: "/admin/content", label: "Content", icon: FileText, permission: "content.read" },
  { href: "/admin/homepage", label: "Homepage", icon: Home, permission: "homepage.write" },
  { href: "/admin/media", label: "Media Library", icon: ImageIcon, permission: "media.read" },
  { href: "/admin/seo", label: "SEO Manager", icon: Globe2, permission: "seo.write" },
  { href: "/admin/team", label: "Team & Roles", icon: Users, permission: "team.write" },
    ...
    <div className="admin-root">
      <aside className={`admin-sidebar ${open ? "is-open" : ""}`}>
        ...
        <nav className="admin-nav" aria-label="Admin navigation">
          ...
          {authorizedNavigation.map(({ href, label, icon: Icon }) => {
            ...
            return <Link prefetch={false} key={href} href={href} className={active ? "active" : ""} onClick={() => setOpen(false)}><Icon size={18} /><span>{label}</span>{label === "Inquiry CRM" && <i>Live</i>}</Link>;
          })}
        </nav>
      ...
      <div className="admin-main">
        <header className="admin-topbar">
          ...
          <div className="admin-search"><Search size={17} /><input ref={searchRef} aria-label="Search admin sections" placeholder="Go to a workspace…" value={search} onChange={(event) => setSearch(event.target.value)} />{searchResults.length > 0 && <div className="admin-search-results">{searchResults.map(({ href, label, icon: Icon }) => <Link prefetch={false} key={href} href={href} onClick={() => setSearch("")}><Icon size={16} />{label}</Link>)}</div>}<kbd>/</kbd></div>
