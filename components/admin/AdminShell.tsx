      ...
      <aside className={`admin-sidebar ${open ? "is-open" : ""}`}>
        ...
        <nav className="admin-nav" aria-label="Admin navigation">
          ...
          {authorizedNavigation.map(({ href, label, icon: Icon }) => {
            ...
            return <Link prefetch={false} key={href} href={href} className={active ? "active" : ""} onClick={() => setOpen(false)}><Icon size={18} /><span>{label}</span>{label === "Permintaan buyer" && <i>Baru</i>}</Link>;
          })}
        </nav>
