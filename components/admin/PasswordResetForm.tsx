...
export function UpdatePasswordForm() {
  ...
  return <form onSubmit={submit} className="admin-login-form"><PasswordInput label="New password" name="password" autoComplete="new-password" minLength={10} /><PasswordInput label="Confirm password" name="confirm" autoComplete="new-password" minLength={10} />{error && <p className="admin-form-error">{error}</p>}<button className="admin-primary-button" disabled={pending}>{pending ? <LoaderCircle className="admin-spin" size={17} /> : <KeyRound size={17} />}{pending ? "Saving…" : "Create password"}<ArrowRight size={17} /></button></form>;
}
