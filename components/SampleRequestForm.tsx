...
export function SampleRequestForm({ product, grade, sourcePage = "/products" }: { product?: string; grade?: string; sourcePage?: string }) {
  ...
  const { register, handleSubmit, reset, formState:{ errors, isSubmitting } } = useForm<SampleInput>({ resolver:zodResolver(sampleSchema), defaultValues:{ product:product||"", grade:grade||"", consent:false, sourcePage } });
  ...
  return <form className="quote-form" onSubmit={handleSubmit(submit)} noValidate><div className="form-grid">
    ...
    <label className="quote-honeypot" aria-hidden="true"><input {...register("website")} tabIndex={-1}/></label><input type="hidden" {...register("sourcePage")}/>
  </div>{serverError&&<p className="form-error" role="alert">{serverError}</p>}<button className="btn btn-gold quote-submit" disabled={isSubmitting}>{isSubmitting?<><LoaderCircle className="spin" size={17}/> Sending request</>:<>Request a sample <ArrowRight size={17}/></>}</button><p className="form-note">Sample availability is subject to approval. Shipping fees may apply. Product specifications are confirmed before dispatch.</p></form>;
}
