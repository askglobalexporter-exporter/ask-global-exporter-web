...
export function ProductDetail({ product }: { product: Product }) {
  const related = products.filter((x) => x.slug !== product.slug).slice(0,3);
  return <main className="b2b-product-page">
    ...
    <section className="product-documents"><div className="shell product-two-col"><motion.div {...reveal}><div className="section-label light">Export documentation</div><h2>Prepared around<br/><em>destination needs.</em></h2><p>Availability depends on the destination, shipment, and agreement. Documents are never represented as universally available.</p></motion.div><div className="product-document-list">{["Commercial Invoice","Packing List","Certificate of Origin","Phytosanitary Certificate","Fumigation Certificate — when required","Bill of Lading / Air Waybill","Product Specification Sheet","Certificate of Analysis — when available"].map(x=><motion.div {...reveal} key={x}><FileCheck2/><span>{x}</span></motion.div>)}</div></div></section>
