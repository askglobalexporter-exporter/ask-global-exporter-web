...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const image = seo?.og_image_url ?? "/hero-vanilla.png";
  return { metadataBase:new URL("https://ask-global-exporter-web.vercel.app"),title:{default:title,template:"%s | Ask Global"},description,keywords:seo?.keywords ?? ["Indonesian Trading Company","Agricultural Commodities","Plantation Products","Indonesian Spices","Natural Ingredients","Private Label","Global Trading"],alternates:{canonical:seo?.canonical_url ?? "/"},robots:seo?.robots ?? "index,follow",openGraph:{title,description,type:"website",locale:"en_US",images:[{url:image,alt:title}]},twitter:{card:"summary_large_image",title,description,images:[image]},icons:{icon:"/favicon.svg"} };
}
