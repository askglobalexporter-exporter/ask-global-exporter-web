...
export default async function ProductsAdminPage({ searchParams }: { searchParams: Promise<{ page?: string; edit?: string }> }) {
  ...
  return <><div className="admin-page-head"><div><h1>Katalog produk</h1><p>Tambah dan perbarui produk. Preview akan berubah langsung sebelum Anda menyimpan.</p></div><div className="admin-page-actions"><span className="admin-badge published">{count ?? 0} produk</span></div></div>
    <div className="admin-split"><article className="admin-card"><div className="admin-card-head"><div><h2><Plus size={14} /> Tambah produk</h2><p>Mulai sebagai draft, periksa preview, lalu terbitkan saat siap.</p></div></div><AdminActionForm action={saveProductAction} successMessage="Produk baru berhasil dibuat." className="admin-form"><ProductFields /><AdminLivePreview kind="product" label="Preview produk"/><div className="admin-form-actions"><SubmitButton pendingLabel="Membuat…">Buat produk</SubmitButton></div></AdminActionForm></article>
      <section>
        {selectedProduct && <article className="admin-card admin-list-card-body"><div className="admin-card-head"><div><h2>Edit {selectedProduct.name}</h2><p>Data lengkap dan galeri hanya dimuat untuk produk ini.</p></div><Link className="admin-secondary-button" href={`/admin/products?page=${page}`}>Tutup</Link></div><AdminActionForm action={saveProductAction} successMessage="Perubahan produk berhasil disimpan." className="admin-form"><ProductFields product={selectedProduct} /><AdminLivePreview kind="product" label="Preview perubahan"/><div className="admin-form-actions"><SubmitButton>Simpan perubahan</SubmitButton></div></AdminActionForm><AdminActionForm action={deleteProductAction} successMessage="Produk berhasil dihapus." pendingMessage="Menghapus produk…" className="admin-form-actions"><input type="hidden" name="id" value={selectedProduct.id} /><SubmitButton className="admin-danger-button" pendingLabel="Menghapus…"><Trash2 size={13} />Hapus produk</SubmitButton></AdminActionForm></article>}
        {params.edit && !selectedProduct && <div className="admin-alert error">Produk yang dipilih tidak ditemukan atau sudah dihapus.</div>}
        {products.length ? products.map((product) => <AdminLazyDetails key={product.id} summary={<>{product.primary_image_url ? <Image className="admin-list-thumb" src={imageThumbnailUrl(product.primary_image_url, 96, 96)} alt="" width={48} height={48} sizes="48px" loading="lazy" /> : <span className="admin-list-thumb" />}<div><b>{product.name}</b><small>/{product.slug} · {product.category}</small></div>{product.is_featured && <span className="admin-badge contacted">Prioritas</span>}<span className={`admin-badge ${product.status}`}>{product.status}</span></>}><div className="admin-list-card-body"><p>Data lengkap belum dimuat agar halaman tetap ringan.</p><Link className="admin-primary-button" href={`/admin/products?page=${page}&edit=${product.id}`}>Edit produk</Link></div></AdminLazyDetails>) : <div className="admin-card admin-empty"><Boxes size={24} /><h3>Belum ada produk</h3><p>Gunakan formulir di sebelah kiri untuk menambahkan produk pertama.</p></div>}
