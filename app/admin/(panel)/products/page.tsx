...
export default async function ProductsAdminPage() {
  ...
  const { data } = await supabase.from("products").select("*").order("position").order("updated_at", { ascending: false });
  ...
  return <><div className="admin-page-head"><div><h1>Product catalogue</h1><p>Create, publish, feature, and maintain export-ready product specifications.</p></div><div className="admin-page-actions"><span className="admin-badge published">{products.filter((p) => p.status === "published").length} live</span></div></div>
    <div className="admin-split"><article className="admin-card"><div className="admin-card-head"><div><h2><Plus size={14} /> Add product</h2><p>Start in draft and publish when ready.</p></div></div><form action={saveProductAction} className="admin-form"><ProductFields /><div className="admin-form-actions"><button className="admin-primary-button">Create product</button></div></form></article>
      <section>{products.length ? products.map((product) => <details className="admin-list-card" key={product.id}><summary>{product.primary_image_url ? <Image className="admin-list-thumb" src={product.primary_image_url} alt="" width={48} height={48} unoptimized /> : <span className="admin-list-thumb" />}<div><b>{product.name}</b><small>/{product.slug} · {product.category}</small></div>{product.is_featured && <span className="admin-badge contacted">Featured</span>}<span className={`admin-badge ${product.status}`}>{product.status}</span></summary><div className="admin-list-card-body"><form action={saveProductAction} className="admin-form"><ProductFields product={product} /><div className="admin-form-actions"><button className="admin-primary-button">Save changes</button></div></form><form action={deleteProductAction} className="admin-form-actions"><input type="hidden" name="id" value={product.id} /><button className="admin-danger-button"><Trash2 size={13} />Delete product</button></form></div></details>) : <div className="admin-card admin-empty"><Boxes size={24} /><h3>No database products yet</h3><p>Create the first product or seed the existing vanilla catalogue.</p></div>}</section>
    </div></>;
}
